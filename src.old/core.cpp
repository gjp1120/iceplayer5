...
gboolean ba_vol(gpointer) {
  g_object_set(core.vol, "volume",configure.volume, NULL);
  return FALSE;
}

gboolean change_cue_pos(gpointer) {
  gchar *p = new gchar[STRINGS_LENGTH];
  ...
  gst_element_seek(core.pipeline,1.0,
				   GST_FORMAT_TIME,
				   GST_SEEK_FLAG_FLUSH,
				   GST_SEEK_TYPE_SET,
				   nanosec,
...
void play_uri() {
    ...
    gchar *uri = new gchar[STRINGS_LENGTH];
    ...
    if (strncmp(focus_song,"<cue>",5)) {
        ...
        gchar *file = new gchar[STRINGS_LENGTH];
