...
const gchar *rename_songs(const gchar *old_uri, const gchar *new_name) {
    ...
    const gchar *old_lrc = get_lrc_path(old_uri);
//dir
...
gboolean read_lists() {
    ...
    if (!g_key_file_load_from_file(keyfile,get_lists_file_path(),
...
gboolean save_lists() {
    ...
    gsize length;
    keyfile = g_key_file_new();
    if (!g_key_file_load_from_file(keyfile,get_lists_file_path(),G_KEY_FILE_NONE,NULL)) {
        print_error("无法加载列表文件，请检查列表文件是否完整！\n");
        g_key_file_free(keyfile);
        return FALSE;
    } else {
