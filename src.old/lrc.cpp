...
gchar *searching_lrc = new gchar[STRINGS_LENGTH];
...
void on_downloader_button_search_clicked
(GtkButton *button,
 GtkWidget *iceplayer_lrc_downloader_entry)
{
    ...
    g_thread_create(search_lrc,NULL,FALSE,NULL);
}
...
void on_downloader_list_selection_changed (GtkList *list,GtkWidget *iceplayer_lrc_downloader_list) {
    ...
    if (glist)
	  download_choose_number = atoi(gtk_tree_path_to_string(static_cast<GtkTreePath*>(glist->data)));
    last_timer=0;
}
...
gpointer search_lrc(gpointer) {
  gchar *searching_lrc_pre = new gchar[STRINGS_LENGTH];
  ...
  if (configure.download_lrc_path[0]=='/') {
	...
	if (!strncmp(focus_song,"<cue>",5)) {
	  //cue:
	  gchar *title = new gchar[STRINGS_LENGTH];
	  gchar *performer = new gchar[STRINGS_LENGTH];
	  gchar *cue_uri = new gchar[STRINGS_LENGTH];
  ...
  gchar *playing_song_lrc_path = new gchar[STRINGS_LENGTH];
  ...
  if (!strncmp(focus_song,"<cue>",5)) {
	...
	delete []cue_uri;
        ...
        gchar *playing_song_path = new gchar[STRINGS_LENGTH];
    ...
    const gchar  *hostname = "www.lrc123.com";
    const gchar  *savepath = "/tmp/iceplayer.lrc.search.log";
    unlink(savepath);
    gchar *error = new gchar[STRINGS_LENGTH];

    gchar *filename = new gchar[STRINGS_LENGTH];
    gint n=0,m=0;
    ...
    if (download(hostname,filename,savepath,error)) {
        ...
        if (strcmp(searching_lrc_pre,searching_lrc)!=0) {
            delete []searching_lrc_pre;
            delete []playing_song_lrc_path;
...
const gchar *get_lrc_file_path_at_custom_path(const gchar *song_uri,gint type) {
    gchar *song_lrc_path= (gchar *)g_malloc(STRINGS_LENGTH*sizeof(gchar *));
    switch (type) {
    case 1: {
