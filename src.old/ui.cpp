...
gboolean read_skin_configure() {
    ...
    if (!g_key_file_load_from_file(keyfile,strcat(get_skin_dir(),"皮肤设置.conf"),G_KEY_FILE_NONE,&error)) {
...
gboolean progress_init_refresh(gpointer) {
  static gint a=0;
  a++;
  if (a>=50) {
	a=0;
